    ...
    <div className="min-h-screen flex flex-col bg-base dark:bg-black text-gray-900 dark:text-white font-sans relative overflow-hidden">
      ...
      <div className="flex flex-1 flex-col md:flex-row items-center justify-center w-full max-w-6xl mx-auto py-12 gap-12">
        ...
        <form onSubmit={handleSubmit} className="flex-1 z-10 bg-white dark:bg-gray-900/90 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl p-10 w-full max-w-md flex flex-col gap-6 backdrop-blur-md">
          <h2 className="text-3xl font-extrabold text-center text-prim tracking-tight mb-2">Sign Up</h2>
          <p className="text-center text-gray-500 dark:text-gray-300 mb-4">Sign up to start using personAI</p>
          <label className="block">
            <span className="block mb-1 font-medium text-black">Name</span>
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-black focus:outline-none focus:ring-2 focus:ring-prim" placeholder="Your Name" required />
          </label>
          <label className="block">
          ...
          <button type="submit" className="w-full py-3 rounded-lg bg-prim text-white font-bold text-lg shadow-lg hover:bg-purple-700 transition" disabled={loading}>{loading ? "Signing up..." : "Sign Up"}</button>
          <div className="text-center text-gray-500 dark:text-gray-300 mt-2">
            Already have an account? <Link href="/login" className="text-prim font-semibold hover:underline">Login</Link>
          </div>
        </form>
      </div>
      {/* Footer */}
